...
=== How do I select a specific Java version for my base image?

By default, JKube uses the `jkube-java` base image which tracks the latest Java version.
To pin a specific Java version, set the `jkube.java.version` property.
Supported values: `11`, `17`, `21`, `25`.
...
This property works identically for Maven and Gradle, and across all Java-based generators.
If both `from` and `jkube.java.version` are set, `from` takes precedence.
See <<generator-java-exec-java-version>> for more details.
