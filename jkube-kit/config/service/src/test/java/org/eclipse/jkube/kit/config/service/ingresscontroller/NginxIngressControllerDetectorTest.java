...
class NginxIngressControllerDetectorTest {
  private KubernetesMockServer mockServer;
  private KubernetesClient client;
  private NginxIngressControllerDetector detector;

  @BeforeEach
  void setUp() {
    detector = new NginxIngressControllerDetector(client);
  }
  ...
  void isDetected_whenIngressClassPresentAndControllerRunning_thenReturnTrue() {
    ...
    mockServer.expect().get()
        .withPath("/apis/networking.k8s.io/v1/ingressclasses?labelSelector=app.kubernetes.io%2Fname%3Dingress-nginx")
        .andReturn(HttpURLConnection.HTTP_OK, new IngressClassListBuilder()
            .addToItems(new IngressClassBuilder().build())
            .build())
        .once();
    mockServer.expect().get()
        .withPath("/api/v1/namespaces/ingress-nginx/pods?labelSelector=app.kubernetes.io%2Fname%3Dingress-nginx")
