...
The *kubernetes-maven-plugin* (k8-m-p) brings your Java applications on to http://kubernetes.io/[Kubernetes].
...
These image related features are inherited from the https://github.com/jshiftio/docker-maven-plugin[jshiftio/docker-maven-plugin] which is part of this plugin.
...
* *XML plugin configuration* mode is similar to what https://jshiftio.github.io/docker-maven-plugin/#example[docker-maven-plugin] provides.
...
| Similarly to the docker-maven-plugin, jshift-m-p supports <<external-dockerfile,external Dockerfiles>> too, which are referenced from the plugin configuration.
...
This minimal but full working example `pom.xml` shows how a simple spring boot application can be dockerized and prepared for Kubernetes. The full example can be found in directory https://github.com/jshiftio/kubernetes-maven-plugin/tree/master/samples/zero-config[samples/zero-config].
...
<project>
  ...
  <build>
    <plugins>
      ...
      <plugin>
        <groupId>io.jshift</groupId>
        <artifactId>k8s-maven-plugin</artifactId> <!--4-->
        <version>{version}</version>
      </plugin>
    </plugins>
  </build>
...
* As base image https://github.com/jshiftio-images/java/tree/master/images/jboss/openjdk8/jdk[jshift.java-jboss-openjdk8-jdk] is chosen which enables https://www.jolokia.org[Jolokia] and https://github.com/prometheus/jmx_exporter[jmx_exporter]. It also comes with a sophisticated https://github.com/jshiftio-images/run-java-sh[startup script].
...
Of course you can bind all those jshift.goals to execution phases as well, so that they are called along with standard lifecycle goals like `install`. For example, to bind the building of the Kubernetes resource files and the Docker images, add the following goals to the execution of the f-m-p:
...
https://jshiftio.github.io/docker-maven-plugin/#configuration[XML configuration] used by `docker-maven-plugin`.
...
* `<images>` defines which Docker <<image-configuration,images>> are used and configured. This section is similar to the https://jshiftio.github.io/docker-maven-plugin/#image-configuration[image configuration] of the `docker-maven-plugin`, except that `<run>` and `<external>` sub-elements are ignored)
...
A working example can be found in the  https://github.com/jshiftio/kubernetes-maven-plugin/tree/master/samples/xml-config[samples/xml-config] directory. An extract of the plugin configuration is shown below:
...
The third configuration option is to use an external configuration in form of YAML resource descriptors which are located in the  `src/main/jshift` directory. Each resource get its own file, which contains a skeleton of a resource descriptor. The plugin will pick up the resource, enrich it and then combine all to a single `kubernetes.yml` and `openshift.yml` file. Within these descriptor files you are can freely use any Kubernetes feature.
...
NOTE: Now that sidecar containers are supported in this plugin(if `jshift.sidecar` is enabled), be careful whenever you're supplying container name in the resource fragment. If container specified in resource fragment doesn't have a name or it's name is equal to default fmp generated application's container name; it would not be treated as sidecar and it would be merged into main container. However, You can override plugin's default name for main container via `jshift.generator.alias` property.
