...
The <<jkube:push>> and <<jkube:build>> goals support several authentication methods for Google Artifact Registry.
...
<plugin>
  <configuration>
    <authConfig>
      ...
      <password>${gcloud.access.token}</password>
    </authConfig>
  </configuration>
</plugin>
----
...
[source,sh,subs="+attributes"]
----
mvn {goal-prefix}:push -Dgcloud.access.token="$(gcloud auth print-access-token)"
----
...
<servers>
  <server>
    <id>us-central1-docker.pkg.dev</id>
