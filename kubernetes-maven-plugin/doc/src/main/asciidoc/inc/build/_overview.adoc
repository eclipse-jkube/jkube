...
If you want to add some `<run>` configuration to this image for starting it with `{plugin}:run` then you can add an image configuration but without a `<build>` section in which case the Dockerfile will be picked up, too. This works only for a single image, though.
