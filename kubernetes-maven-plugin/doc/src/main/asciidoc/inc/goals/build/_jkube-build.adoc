...
There are three kind of binary builds supported by this plugin, which can be selected with the `buildStrategy` configuration option (`jkube.build.strategy` property)
...
| *logDate*
| The date format to use when putting out logs. Default is `DEFAULT` (`HH:mm:ss.SSS`)
