...
Images can be built in two different ways depending on the `mode` configuration (controlled by the `jkube.mode`
property).
By default the mode is set to `auto`. In this case the plugin tries to detect which kind of build should be performed by
contacting the API server. If this fails or if no cluster access is configured e.g. with `oc login` then the mode is set
to `kubernetes` in which case a standard Docker build is performed. It can also be forced to `openshift` to perform an
OpenShift build.
endif::[]
...
If the mode is set to `kubernetes` then a normal Docker build is performed. The connection configuration to access the
Docker daemon is described in <<access-configuration, Access Configuration>>.
