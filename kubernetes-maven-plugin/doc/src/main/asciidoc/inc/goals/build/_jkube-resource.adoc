...
| Environment name where resources are placed. For example, if you set this property to dev and resourceDir is the
  ...
  *Merge Behavior for YAML/JSON Files:*
  ...
  * *Arrays/Lists*: Elements from the new fragment are *appended* to the existing list (not merged by identity). This means same-identity list elements (containers, env vars, ports) will be duplicated rather than merged.
  * *Scalar Values*: New values replace existing values

  *Example Use Case:*

  Given `common/route.yml`:
  ```yaml
  metadata:
    annotations:
  ...
  metadata:
    annotations:
      cert-manager.io/issuer-kind: ClusterIssuer  # from common (preserved)
      cert-manager.io/issuer-name: letsencrypt-prod # from common (preserved)
    namespace: demo-namespace                       # from dev (added)
  spec:
    host: demoapp.apps.example.com                  # from dev (added)
    tls:
      termination: reencrypt                        # from dev (overrides common)
      insecureEdgeTerminationPolicy: Allow          # from dev (overrides common)
  ```
