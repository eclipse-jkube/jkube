...
| Environment name where resources are placed. For example, if you set this property to dev and resourceDir is the
  ...
  When multiple environments are specified, resource fragments in these directories are intelligently combined:

  * Fragments with the same filename from different environment directories are *deeply merged* at the property level for YAML files
  * This allows you to define base configurations in one directory (e.g., `common/`) and override or extend specific properties in environment-specific directories (e.g., `dev/`, `prod/`)
  * Example: A `route.yml` in `common/` can define base TLS settings, while `route.yml` in `dev/` can add environment-specific annotations or host - the final resource will contain merged properties from both files
