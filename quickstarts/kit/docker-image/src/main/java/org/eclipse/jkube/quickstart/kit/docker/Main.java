...
public class Main {
  public static void main(String[] args) {
    ...
    final JKubeConfiguration configuration = new JKubeConfiguration.Builder()
        .project(JavaProject.builder().build())
        .outputDirectory(new File("target").getAbsolutePath())
        .build();
    final JKubeServiceHub jKubeServiceHub = new JKubeServiceHub.Builder()
        .log(kitLogger)
        .configuration(configuration)
        .platformMode(RuntimeMode.kubernetes)
        .dockerServiceHub(serviceHub)
        .build();
    jKubeServiceHub.close();
  }
//
//
