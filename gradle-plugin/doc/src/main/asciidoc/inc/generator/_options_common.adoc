...
| When set to `true`, then the generator _adds_ to an existing image configuration. By default, this is disabled, so
  ...
  directly for a `{task-prefix}Build` or already added by a generator which has been run previously.
| `jkube.generator.add`
...
| *registry*
| An optional Docker registry used when doing Docker builds. It has no effect for OpenShift S2I builds.
