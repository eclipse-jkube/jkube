...
import static io.jshift.maven.enricher.api.util.KubernetesResourceUtil.API_VERSION;
import static io.jshift.maven.enricher.api.util.KubernetesResourceUtil.DEFAULT_RESOURCE_VERSIONING;
import static io.jshift.maven.enricher.api.util.KubernetesResourceUtil.getResource;
...
public class KubernetesResourceUtilTest {
    ...
    public static void initPath() throws UnsupportedEncodingException {
        ...
        String filePath = URLDecoder.decode(classLoader.getResource("jshift/simple-rc.yaml").getFile(), "UTF-8");
        jshiftDir = new File(filePath).getParentFile();
    }

    @Test
    public void simple() throws IOException {
        for (String ext : new String[] { "yaml", "json" }) {
            HasMetadata ret = getResource(PlatformMode.kubernetes, DEFAULT_RESOURCE_VERSIONING, new File(jshiftDir, "simple-rc." + ext), "app");
    ...
    public void withValue() throws IOException {
        HasMetadata ret = getResource(PlatformMode.kubernetes, DEFAULT_RESOURCE_VERSIONING, new File(jshiftDir, "named-svc.yaml"), "app");
        assertEquals(API_VERSION, ret.getApiVersion());
    ...
    public void invalidType() throws IOException {
        try {
            getResource(PlatformMode.kubernetes, DEFAULT_RESOURCE_VERSIONING, new File(jshiftDir, "simple-bla.yaml"), "app");
            fail();
    ...
    public void containsKind() throws Exception {
        HasMetadata ret = getResource(PlatformMode.kubernetes, DEFAULT_RESOURCE_VERSIONING, new File(jshiftDir, "contains_kind.yml"), "app");
        assertEquals("ReplicationController", ret.getKind());
    }
    ...
    public void job() throws Exception {
        HasMetadata ret = getResource(PlatformMode.kubernetes, DEFAULT_RESOURCE_VERSIONING, new File(jshiftDir, "job.yml"), "app");
    ...
    public void containsNoKindAndNoTypeInFilename() throws Exception {
        try {
            getResource(PlatformMode.kubernetes, DEFAULT_RESOURCE_VERSIONING, new File(jshiftDir, "contains_no_kind.yml"), "app");
            fail();
        } catch (IllegalArgumentException exp) {
    ...
    public void invalidPattern() throws IOException {
        try {
            getResource(PlatformMode.kubernetes, DEFAULT_RESOURCE_VERSIONING, new File(jshiftDir, "blubber.yaml"), "app");
            fail();
    ...
    public void noNameInFile() throws IOException {
        HasMetadata ret = getResource(PlatformMode.kubernetes, DEFAULT_RESOURCE_VERSIONING, new File(jshiftDir, "rc.yml"), "app");
        assertEquals("flipper",ret.getMetadata().getName());
    }

    @Test
    public void noNameInFileAndNotInMetadata() throws IOException {
        HasMetadata ret = getResource(PlatformMode.kubernetes, DEFAULT_RESOURCE_VERSIONING, new File(jshiftDir, "svc.yml"), "app");
        assertEquals("Service",ret.getKind());
    ...
    public void invalidExtension() throws IOException {
        try {
            getResource(PlatformMode.kubernetes, DEFAULT_RESOURCE_VERSIONING, new File(jshiftDir, "simple-rc.txt"), "app");
            fail();
        } catch (IllegalArgumentException exp) {
    ...
    public void readWholeDir() throws IOException {
        ...
        KubernetesListBuilder builder =
            KubernetesResourceUtil.readResourceFragmentsFrom(PlatformMode.kubernetes, v, "pong", new File(jshiftDir, "read-dir").listFiles());
