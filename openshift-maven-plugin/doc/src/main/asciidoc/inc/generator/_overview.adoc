The usual way to define Docker images is with the plugin configuration as explained in <<oc:build>>. This can either be done completely within the `pom.xml` or by referring to an external Dockerfile. Since kubernetes-maven-plugin includes https://github.com/jshiftio/docker-maven-plugin[docker-maven-plugin] the way by which images are built is identical.
